...
namespace Hmxs.Toolkit.Quests.MonoClass
{
    ...
    public class QuestManager : MonoBehaviour
    {
        ...
        {
            EventCenter.AddListener<string>(QuestEvents.QuestStart, OnQuestStart);
            EventCenter.AddListener<string>(QuestEvents.QuestAdvance, OnQuestAdvance);
            EventCenter.AddListener<string>(QuestEvents.QuestFinish, OnQuestFinish);
            EventCenter.AddListener<string, int, QuestStepData>(QuestEvents.QuestStepDataUpdate, OnStepDataUpdate);
            // 玩家Level改变时触发回调
            PlayerData.OnLevelChange += CheckAllQuestsRequirements;
        }

        private void OnDisable()
        {
            EventCenter.RemoveListener<string>(QuestEvents.QuestStart, OnQuestStart);
            EventCenter.RemoveListener<string>(QuestEvents.QuestAdvance, OnQuestAdvance);
            EventCenter.RemoveListener<string>(QuestEvents.QuestFinish, OnQuestFinish);
            EventCenter.RemoveListener<string, int, QuestStepData>(QuestEvents.QuestStepDataUpdate, OnStepDataUpdate);
            PlayerData.OnLevelChange -= CheckAllQuestsRequirements;
        }
        #endregion
        ...
        private void Start()
        {
            ...
            foreach (var quest in _questMap.Values)
            {
                ...
                EventCenter.Trigger(QuestEvents.QuestStateChange, quest);
            }
            // 开始运行时先检查一次
        ...
        {
            QuestInfoSO[] allQuestInfo = Resources.LoadAll<QuestInfoSO>($"Quests");
        ...
        private bool CheckRequirementsMetState(Quest quest)
        {
            var isMet =
                ...
                // 这里以Level为例
                !(PlayerData.Level < quest.Info.levelRequirement);
        ...
        {
            // todo:获取报酬的逻辑
            PlayerData.Level += quest.Info.experienceReward;
        }
