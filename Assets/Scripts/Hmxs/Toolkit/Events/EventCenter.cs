...
namespace Hmxs.Toolkit.Events
{
    ...
    public static class EventCenter
    {
        ...
        {
            if (EventInfos.TryGetValue(eventName, out var value)) 
                ((EventInfo<T>)value).Action -= action;
        }
        ...
        {
            if (EventInfos.TryGetValue(eventName, out var value)) 
                ((EventInfo<T>)value).Action?.Invoke(param);
            else
                Debug.LogWarning($"EventCenter: Null {eventName} is listened");
        }
        ...
        #region Delete Event/Clear

        /// <summary>
        /// 删除某一事件
        /// </summary>
        public static bool DeleteEvent(string eventName)
        {
            return EventInfos.Remove(eventName);
        }
